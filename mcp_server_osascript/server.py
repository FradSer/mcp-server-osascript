...
def execute_osascript_direct(script: str, timeout: int = 20) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if tcc_error:
                # If TCC error detected, try to trigger dialog via terminal
                logger.info("TCC permission denied, attempting to trigger dialog")
                terminal_result = try_trigger_tcc_via_terminal(script, tcc_error)
                if terminal_result:
...
def try_trigger_tcc_via_terminal(script: str, tcc_error: Dict[str, str]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.command', delete=False) as f:
            ...
            f.write(f'echo ""\n')
            f.write(f'osascript -e \'{script.replace("'", "'\''")}\'\n')
            f.write(f'echo ""\n')
    ...
    try:
        ...
        tell application "Terminal"
            activate
            do script "osascript -e '{script.replace("'", "'\''")}'"
        end tell
        '''
